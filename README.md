...
- Indonesian NIK/KTP numbers with a valid encoded birth date.
...
You can also reorder detectors with drag and drop; add custom regex detectors;
edit their name, regex, and placeholder prefix; enable or disable them; delete
them; and test a regex against a sample value before saving. **Custom values**
and **Columns to mask** are included in the collapsed detection settings panel.
All detection settings can be exported to JSON and imported in another browser.
