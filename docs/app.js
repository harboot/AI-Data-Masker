(() => {
  ...
  const BUILTIN_DETECTORS = [
    { id:'email', name:'Email address', description:'Email addresses with a valid local part and domain.', prefix:'EMAIL', pattern:"\\b[A-Z0-9.!#$%&'*+/=?^_`{|}~-]+@[A-Z0-9](?:[A-Z0-9-]{0,61}[A-Z0-9])?(?:\\.[A-Z0-9](?:[A-Z0-9-]{0,61}[A-Z0-9])?)+\\b", valid:()=>true },
    { id:'ip', name:'IPv4 & IPv6 address', description:'Valid IPv4 and IPv6 network addresses.', prefix:'IP', pattern:'(?<![\\w.])(?:\\d{1,3}\\.){3}\\d{1,3}(?![\\w.])|(?<![\\w:])(?:[0-9a-f]{0,4}:){2,7}[0-9a-f:.]{0,15}(?![\\w:])', valid:value=>value.includes(':')?validIPv6(value):validIPv4(value) },
    { id:'domain', name:'Supported domain', description:'Domain names ending in .com, .com.ph, .org, or .net.', prefix:'DOMAIN', pattern:'(?<![@\\w-])(?:[a-z0-9](?:[a-z0-9-]{0,61}[a-z0-9])?\\.)+(?:com\\.ph|com|org|net)\\b', valid:()=>true },
    { id:'card', name:'Luhn-valid credit card', description:'Credit card numbers containing 13–19 digits that pass Luhn validation.', prefix:'CARD', pattern:'(?<!\\d)(?:\\d[ -]?){12,18}\\d(?!\\d)', valid:value=>{const n=value.replace(/\D/g,'');if(n.length<13||n.length>19)return false;let sum=0,alt=false;for(let i=n.length-1;i>=0;i--){let d=+n[i];if(alt&&(d*=2)>9)d-=9;sum+=d;alt=!alt;}return sum%10===0;} }
  ];
  const validators = {
    nik:value=>{const day=+value.slice(6,8),month=+value.slice(8,10);return (day>=1&&day<=31||day>=41&&day<=71)&&month>=1&&month<=12;}
  };
  ...
  function renderDetectors() {
    ...
    detectors.forEach(detector=>{
      ...
      const info=document.createElement('div');info.className='detector-info';const title=document.createElement('div');title.className='detector-name';title.textContent=detector.name;const kind=document.createElement('span');kind.className='detector-kind';kind.textContent=detector.builtin?'Built-in':detector.source==='predefined'?`Predefined · ${detector.prefix}`:`Custom · ${detector.prefix}`;title.append(kind);info.append(title);row.append(handle,toggle,details,info);
  ...
  function openRegexDetails(detector){$('regexDetailsTitle').textContent=detector.name;$('regexDetailsDescription').value=detector.description||'No description provided.';$('regexDetailsPrefix').value=detector.prefix;$('regexDetailsPattern').value=detector.pattern;$('regexDetailsSample').value='';const result=$('regexDetailsTestResult');result.textContent='';result.className='test-result';$('regexDetailsDialog').showModal();}
  ...
  $('detectorForm').addEventListener('submit',event=>{event.preventDefault();const id=$('detectorId').value,name=$('detectorName').value.trim(),description=$('detectorDescription').value.trim(),pattern=$('detectorPattern').value,prefix=$('detectorPrefix').value.trim().toUpperCase();try{new RegExp(pattern,'gi');}catch(err){setTestResult(`Invalid regex: ${err.message}`,'error');return;}const duplicate=BUILTIN_DETECTORS.some(d=>d.prefix===prefix)||detectorSettings.custom.some(d=>d.prefix===prefix&&d.id!==id);if(duplicate){setTestResult('Prefix must be unique.','error');return;}if(id){const detector=detectorSettings.custom.find(d=>d.id===id);Object.assign(detector,{name,description,pattern,prefix});}else{detectorSettings.custom.push({id:`custom-${Date.now()}-${Math.random().toString(36).slice(2,7)}`,name,description,pattern,prefix,enabled:true});}saveDetectorSettings();renderDetectors();closeDetectorDialog();});
  ...
  $('settingsFile').addEventListener('change',async event=>{const file=event.target.files[0];if(!file)return;try{const imported=JSON.parse(await file.text());if(!imported||!Array.isArray(imported.custom)||imported.custom.some(d=>!isValidCustomDetector(d)))throw new Error('Invalid settings file.');detectorSettings.disabled=Array.isArray(imported.disabled)?imported.disabled.filter(id=>typeof id==='string'):[];detectorSettings.custom=imported.custom.map(d=>({...d,description:typeof d.description==='string'?d.description:''}));detectorSettings.deletedPredefined=Array.isArray(imported.deletedPredefined)?imported.deletedPredefined.filter(id=>typeof id==='string'):[];detectorSettings.order=Array.isArray(imported.order)?imported.order.filter(id=>typeof id==='string'):[];detectorSettings.columns=typeof imported.columns==='string'?imported.columns:'';detectorSettings.values=typeof imported.values==='string'?imported.values:'';$('columnHeaders').value=detectorSettings.columns;$('customValues').value=detectorSettings.values;saveDetectorSettings();renderDetectors();$('settingsStatus').style.color='';$('settingsStatus').textContent='Settings imported successfully.';}catch(err){$('settingsStatus').textContent=err.message||'Unable to import settings.';$('settingsStatus').style.color='var(--bad)';}});
  ...
  async function initializeDetectors() {
    try {
      ...
      const existing=new Set(detectorSettings.custom.map(d=>d.id));
      predefined.filter(d=>!existing.has(d.id)&&!detectorSettings.deletedPredefined.includes(d.id)).forEach(d=>detectorSettings.custom.push({...d,source:'predefined',enabled:true}));
    ...
    $('columnHeaders').value=detectorSettings.columns;$('customValues').value=detectorSettings.values;renderDetectors();
  }
  function createMasker(customInput) {
    ...
    const definitions = [
      ...
      ...[...BUILTIN_DETECTORS.map(d=>({...d,enabled:!detectorSettings.disabled.includes(d.id)})),...detectorSettings.custom.map(d=>({...d,valid:validators[d.validator]||(()=>true)}))]
