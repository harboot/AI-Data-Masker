(() => {
  ...
  const headerDetectors = () => detectorSettings.custom.filter(d=>d.kind==='header'&&d.enabled!==false);
  ...
  function loadDetectorSettings() {
    try {
      ...
      if (saved && typeof saved === 'object' && Array.isArray(saved.custom)) return { disabled:Array.isArray(saved.disabled)?saved.disabled:[], custom:saved.custom.filter(isValidCustomDetector).map(d=>({...d,kind:d.kind||'regex',group:typeof d.group==='string'?d.group:'Custom',description:typeof d.description==='string'?d.description:''})), deletedPredefined:Array.isArray(saved.deletedPredefined)?saved.deletedPredefined.filter(id=>typeof id==='string'):[], order:Array.isArray(saved.order)?saved.order.filter(id=>typeof id==='string'):[], legacyColumns:typeof saved.columns==='string'?saved.columns:'', values:typeof saved.values==='string'?saved.values:'' };
    } catch (_) { /* Ignore unavailable storage or invalid saved settings. */ }
    return { disabled:[], custom:[], deletedPredefined:[], order:[], legacyColumns:'', values:'' };
  }
  function isValidCustomDetector(detector) {
    if (!detector || typeof detector.id !== 'string' || typeof detector.name !== 'string' || !/^[A-Za-z][A-Za-z0-9_]*$/.test(detector.prefix || '')) return false;
    if (detector.description != null && typeof detector.description !== 'string') return false;
    if(detector.group != null&&typeof detector.group!=='string')return false;
    if(detector.kind==='header')return typeof detector.header==='string'&&Boolean(detector.header.trim());
    if(typeof detector.pattern!=='string')return false;
  ...
  function renderDetectors() {
    ...
    detectors.forEach(detector=>{
      ...
      const details=document.createElement('button');details.type='button';details.className='regex-details-button';details.textContent='👁';details.title=detector.kind==='header'?`View ${detector.name} column header`:`View and test ${detector.name} regex`;details.setAttribute('aria-label',details.title);details.addEventListener('click',()=>openRegexDetails(detector));
      const info=document.createElement('div');info.className='detector-info';const title=document.createElement('div');title.className='detector-name';title.textContent=detector.name;const kind=document.createElement('span');kind.className='detector-kind';kind.textContent=detector.builtin?'Built-in':detector.kind==='header'?`Header · ${detector.header}`:detector.source==='predefined'?`Predefined · ${detector.prefix}`:`Regex · ${detector.prefix}`;title.append(kind);info.append(title);row.append(handle,toggle,details,info);
      if(!detector.builtin){const actions=document.createElement('div');actions.className='detector-actions';const edit=document.createElement('button');edit.type='button';edit.className='secondary';edit.textContent='Edit';edit.addEventListener('click',()=>openDetectorDialog(detector));const remove=document.createElement('button');remove.type='button';remove.className='secondary danger';remove.textContent='Delete';remove.addEventListener('click',()=>{if(confirm(`Delete custom detector “${detector.name}”?`)){detectorSettings.custom=detectorSettings.custom.filter(d=>d.id!==detector.id);if(detector.source==='predefined')detectorSettings.deletedPredefined=[...new Set([...detectorSettings.deletedPredefined,detector.id])];detectorSettings.order=detectorSettings.order.filter(id=>id!==detector.id);saveDetectorSettings();renderDetectors();}});actions.append(edit,remove);row.append(actions);}list.append(row);
  ...
  function openRegexDetails(detector){$('regexDetailsTitle').textContent=detector.name;$('regexDetailsDescription').value=detector.description||'No description provided.';$('regexDetailsPrefix').value=detector.prefix;$('regexDetailsPattern').value=detector.kind==='header'?`Column header: ${detector.header}`:detector.pattern;$('regexDetailsSample').value='';$('regexDetailsSample').disabled=detector.kind==='header';$('testRegexDetailsButton').disabled=detector.kind==='header';const result=$('regexDetailsTestResult');result.textContent=detector.kind==='header'?'This detector matches the exact column header shown above.':'';result.className='test-result';$('regexDetailsDialog').showModal();}
  ...
  $('addDetectorButton').addEventListener('click',()=>openDetectorDialog());$('detectorType').addEventListener('change',updateDetectorType);$('closeDetectorDialog').addEventListener('click',closeDetectorDialog);$('cancelDetectorButton').addEventListener('click',closeDetectorDialog);$('testDetectorButton').addEventListener('click',testDetector);$('testRegexDetailsButton').addEventListener('click',testRegexDetails);
  $('detectorForm').addEventListener('submit',event=>{event.preventDefault();const id=$('detectorId').value,name=$('detectorName').value.trim(),description=$('detectorDescription').value.trim(),group=$('detectorGroup').value.trim()||'Custom',kind=$('detectorType').value,pattern=$('detectorPattern').value,header=$('detectorHeader').value.trim(),prefix=$('detectorPrefix').value.trim().toUpperCase();if(kind==='regex'){if(!pattern){setTestResult('Regex is required.','error');return;}try{new RegExp(pattern,'gi');}catch(err){setTestResult(`Invalid regex: ${err.message}`,'error');return;}}else if(!header){setTestResult('Column header is required.','error');return;}const duplicate=BUILTIN_DETECTORS.some(d=>d.prefix===prefix)||detectorSettings.custom.some(d=>d.prefix===prefix&&d.id!==id);if(duplicate){setTestResult('Prefix must be unique.','error');return;}const values={name,description,group,kind,prefix,...(kind==='header'?{header}:{pattern})};if(id){const detector=detectorSettings.custom.find(d=>d.id===id),enabled=detector.enabled,source=detector.source;Object.keys(detector).forEach(key=>{if(!['id'].includes(key))delete detector[key];});Object.assign(detector,values,{enabled,source,modified:source==='predefined'||undefined});}else{detectorSettings.custom.push({id:`custom-${Date.now()}-${Math.random().toString(36).slice(2,7)}`,...values,enabled:true});}saveDetectorSettings();renderDetectors();closeDetectorDialog();});
  $('customValues').addEventListener('input',()=>{detectorSettings.values=$('customValues').value;saveDetectorSettings();});
  $('exportSettingsButton').addEventListener('click',()=>{detectorSettings.values=$('customValues').value;const url=URL.createObjectURL(new Blob([JSON.stringify({version:2,...detectorSettings},null,2)],{type:'application/json'})),link=document.createElement('a');state.urls.push(url);link.href=url;link.download='ai-data-masker-settings.json';link.click();$('settingsStatus').textContent='Settings exported.';});
  $('importSettingsButton').addEventListener('click',()=>{$('settingsFile').value='';$('settingsFile').click();});
  $('settingsFile').addEventListener('change',async event=>{const file=event.target.files[0];if(!file)return;try{const imported=JSON.parse(await file.text());if(!imported||!Array.isArray(imported.custom)||imported.custom.some(d=>!isValidCustomDetector(d)))throw new Error('Invalid settings file.');detectorSettings.disabled=Array.isArray(imported.disabled)?imported.disabled.filter(id=>typeof id==='string'):[];detectorSettings.custom=imported.custom.map(d=>({...d,kind:d.kind||'regex',group:typeof d.group==='string'?d.group:'Custom',description:typeof d.description==='string'?d.description:''}));detectorSettings.deletedPredefined=Array.isArray(imported.deletedPredefined)?imported.deletedPredefined.filter(id=>typeof id==='string'):[];detectorSettings.order=Array.isArray(imported.order)?imported.order.filter(id=>typeof id==='string'):[];detectorSettings.legacyColumns=typeof imported.columns==='string'?imported.columns:'';detectorSettings.values=typeof imported.values==='string'?imported.values:'';$('customValues').value=detectorSettings.values;saveDetectorSettings();await initializeDetectors();$('settingsStatus').style.color='';$('settingsStatus').textContent='Settings imported successfully.';}catch(err){$('settingsStatus').textContent=err.message||'Unable to import settings.';$('settingsStatus').style.color='var(--bad)';}});
  const detectorReady = initializeDetectors();
  async function initializeDetectors() {
    try {
      const response=await fetch('predefine.json');if(!response.ok)throw new Error(`Unable to load predefine.json (${response.status}).`);
      const predefined=await response.json();if(!Array.isArray(predefined)||predefined.some(d=>!isValidCustomDetector(d)))throw new Error('predefine.json is invalid.');
      const oldById=new Map(detectorSettings.custom.map(d=>[d.id,d]));
      detectorSettings.custom=detectorSettings.custom.filter(d=>d.source!=='predefined'&&!predefined.some(p=>p.id===d.id));
      predefined.filter(d=>!detectorSettings.deletedPredefined.includes(d.id)).forEach(d=>{const old=oldById.get(d.id);detectorSettings.custom.push(old?.modified?old:{...d,kind:'regex',source:'predefined',enabled:old?.enabled!==false});});
      if(detectorSettings.legacyColumns){for(const header of [...new Set(detectorSettings.legacyColumns.split(/[\n,]+/).map(v=>v.trim()).filter(Boolean))]){const prefix=`COLUMN_${header.replace(/[^A-Za-z0-9_]/g,'_').replace(/^[^A-Za-z]+/,'')||'VALUE'}`.toUpperCase();if(!detectorSettings.custom.some(d=>d.kind==='header'&&d.header===header))detectorSettings.custom.push({id:`header-${Date.now()}-${Math.random().toString(36).slice(2,7)}`,name:`Column: ${header}`,description:`Masks every non-empty value in the ${header} column.`,group:'Columns',kind:'header',header,prefix,enabled:true});}detectorSettings.legacyColumns='';}
      saveDetectorSettings();
    } catch(err) { $('settingsStatus').style.color='var(--bad)';$('settingsStatus').textContent=err.message||'Unable to load predefined detectors.'; }
    $('customValues').value=detectorSettings.values;renderDetectors();
  }
  function createMasker(customInput) {
    ...
    const definitions = [
      ...
      ...[...BUILTIN_DETECTORS.map(d=>({...d,enabled:!detectorSettings.disabled.includes(d.id)})),...detectorSettings.custom.map(d=>({...d,valid:()=>true}))]
        .filter(d=>d.kind!=='header'&&d.enabled!==false).sort((a,b)=>detectorSettings.order.indexOf(a.id)-detectorSettings.order.indexOf(b.id))
    ...
    function maskColumn(value,detector){if(!value)return value;const type=detector.prefix,key=`${detector.header}\0${value}`;let byValue=maps.get(type);if(!byValue){byValue=new Map();maps.set(type,byValue);}let token=byValue.get(key);if(!token){token=`[${type}_${String(byValue.size+1).padStart(3,'0')}]`;byValue.set(key,token);mappings.push({token,original:value,type,header:detector.header});}counts[type]=(counts[type]||0)+1;return token;}
  ...
  $('maskButton').addEventListener('click', async () => {
    ...
    try{
      const masker=createMasker($('customValues').value), parts=[],wanted=headerDetectors();let carry='',headerMatches=null,delimiter=null,preview='';
      await readTextChunks(file,async(text,last)=>{carry+=text;let cut=last?carry.length:carry.lastIndexOf('\n')+1;if(cut>0){let piece=carry.slice(0,cut);carry=carry.slice(cut);if(wanted.length){const lines=piece.split(/(?<=\n)/);for(let i=0;i<lines.length;i++){const ending=(lines[i].match(/\r?\n$/)||[''])[0];let line=lines[i].slice(0,lines[i].length-ending.length);if(headerMatches===null){delimiter=delimiterFor(line,file.name);if(!delimiter)throw new Error('Column-header detectors require CSV or TSV data.');const headers=parseRow(line,delimiter).map(c=>c.value.trim());const missing=wanted.filter(d=>!headers.includes(d.header));if(missing.length)throw new Error(`Column header not found: ${missing.map(d=>d.header).join(', ')}`);headerMatches=wanted.map(d=>({index:headers.indexOf(d.header),detector:d}));}else{const cells=parseRow(line,delimiter),replacements=headerMatches.filter(match=>cells[match.index]).map(match=>({cell:cells[match.index],value:masker.maskColumn(cells[match.index].value,match.detector)})).sort((a,b)=>b.cell.start-a.cell.start);for(const r of replacements){const raw=line.slice(r.cell.start,r.cell.end),quoted=raw.startsWith('"')&&raw.endsWith('"');line=line.slice(0,r.cell.start)+(quoted?`"${r.value}"`:r.value)+line.slice(r.cell.end);}}lines[i]=line+ending;}piece=lines.join('');}const maskedPiece=masker.mask(piece);parts.push(maskedPiece);if(preview.length<65536)preview+=(maskedPiece.slice(0,65536-preview.length));}},p=>setProgress('mask',p));
